 ...
 * 
 * [ux.TouchCalendar Demo](http://www.swarmonline.com/ux.TouchCalendar/examples/ux.TouchCalendar.html)
 * 
 */
Ext.define('ux.TouchCalendar',{
	...
	config: {
		...
		*/
		...
		/**
		* @cfg {Boolean/Object} enableEventBars True to enable the ux.TouchCalendarEvents plugin. When true the ux.TouchCalendarEvents JS and CSS files
		* must be included and an eventStore option, containing an Ext.data.Store instance, be given to the viewConfig.  If an object is passed in this is used as the config for the plugin.
		*/
	...
	*/
	getViewConfig: function(viewValue){
		...
		if(this.getEnableSimpleEvents()){
			...
			plugins.push(Ext.create('ux.TouchCalendarSimpleEvents', config));
		} else if (this.getEnableEventBars()){
			var config = Ext.isObject(this.getEnableEventBars()) ? this.getEnableEventBars() : {};
			plugins.push(Ext.create('ux.TouchCalendarEvents', config));
		}
	...
	initViews: function(){
		...
		items.push(
		    Ext.create('ux.TouchCalendarView', Ext.applyIf({
		        currentDate: viewValue
		...
		// active view
		items.push(
			Ext.create('ux.TouchCalendarView', ux.TouchCalendarView(this.getViewConfig(origCurrentDate)))
		);

		// second out of view (i.e. third)
		viewValue = this.getViewDate(origCurrentDate, 1);
		items.push(
			Ext.create('ux.TouchCalendarView', ux.TouchCalendarView(Ext.applyIf({
		        currentDate: viewValue
		    }, this.getViewConfig(viewValue))))
		);
	...
	onActiveItemChange: function(container, newCard, oldCard){
		if (this.getEnableSwipeNavigate()) {
			...
			if (direction === 'forward') {
				...
				var newCalendar = new ux.TouchCalendarView(this.getViewConfig(this.getViewDate(newCard.currentDate, 1)));
...
/**
 * ux.TouchCalendarEventsBase
 */
Ext.define('ux.TouchCalendarEventsBase', {
	...
	generateEventBars: function(){
		 ...
		 * with the ux.CalendarEventBarModel model.
		 * @private
		 */
		this.eventBarStore = Ext.create('Ext.data.Store', {
			model: 'ux.CalendarEventBarModel',
			data: []
		});
			...
			eventStore.each(function(event){
				...
				if (eventBarIndex > -1) {
					...
					if (currentDate.getDay() === this.getCalendar().getWeekStart()) {
						...
						var wrappedEventBarRecord = Ext.create('ux.CalendarEventBarModel', {
					...
					eventBarRecord = Ext.create('ux.CalendarEventBarModel', {
						EventID: event.internalId,
						Date: currentDate,
						BarLength: 1,
						BarPosition: barPos,
	 ...
	 * @method
	 * @private
	 * @param {ux.CalendarEventBarModel} r The EventBar model instance to figure out if it wraps to the next row of dates
	 */
	eventBarDoesWrap: function(r){
		var barEndDate = Ext.Date.add(r.get('Date'), Ext.Date.DAY, (r.get('BarLength') - 1));
		return Ext.Date.clearTime(barEndDate, true).getTime() !== Ext.Date.clearTime(new Date(r.get('Record').get(this.getPlugin().getEndEventField())), true).getTime();
	},
	/**
	 * Returns true if the specified EventBar record has been wrapped from the row before.
	 * @method
	 * @private
	 * @param {ux.CalendarEventBarModel} r The EventBar model instance to figure out if it has wrapped from the previous row of dates
	 */
	eventBarHasWrapped: function(r){
		return Ext.Date.clearTime(r.get('Date'), true).getTime() !== Ext.Date.clearTime(new Date(r.get('Record').get(this.getPlugin().getStartEventField())), true).getTime();
	},
 ...
 * ux.TouchCalendarDayEvents
 */
Ext.define('ux.TouchCalendarDayEvents', {

    extend: 'ux.TouchCalendarEventsBase',
	...
	eventFilterFn: function(record, id, currentDateTime){
		var startDate   = this.getRoundedTime(new Date(record.get(this.getPlugin().getStartEventField()))).getTime(),
			endDate     = this.getRoundedTime(new Date(record.get(this.getPlugin().getEndEventField()))).getTime();
 ...
 * ux.TouchCalendarMonthEvents
 */
Ext.define('ux.TouchCalendarMonthEvents', {

    extend: 'ux.TouchCalendarEventsBase',

    eventFilterFn: function (record, id, currentDateTime) {
		var startDate = Ext.Date.clearTime(new Date(record.get(this.getPlugin().getStartEventField())), true).getTime(),
			endDate = Ext.Date.clearTime(new Date(record.get(this.getPlugin().getEndEventField())), true).getTime();
 ...
 * ux.TouchCalendarWeekEvents
 */
Ext.define('ux.TouchCalendarWeekEvents', {

    extend: 'ux.TouchCalendarMonthEvents'
 ...
 */
/**
 * @class ux.TouchCalendarEvents
 * @author Stuart Ashworth
 *
 * For use with Sencha Touch 2
 * 
 * This plugin also allows a store to be bound to the ux.TouchCalendar and will display the store's events as bars spanning its relevant days. 
 * 
 * ![ux.TouchCalendarEvents Screenshot](http://www.swarmonline.com/ux.TouchCalendar/screenshots/ux.TouchCalendarEvents--month-ss.png)
 * 
 * [ux.TouchCalendarEvents Demo](http://www.swarmonline.com/ux.TouchCalendar/examples/ux.TouchCalendarEvents.html)
 * 
 */
Ext.define('ux.TouchCalendarEvents', {
	...
	getViewModeProcessorClass: function(viewMode){
		...
		switch(viewMode.toLowerCase()){
			...
			case 'month':
				processorCls = 'ux.TouchCalendarMonthEvents';
				break;

			case 'week':
				processorCls = 'ux.TouchCalendarWeekEvents';
				break;

			case 'day':
				processorCls = 'ux.TouchCalendarDayEvents';
				break;
		}
 ...
 * @private
 */
Ext.define("ux.CalendarEventBarModel", {
